...
async function listAutomationYamlFiles() {
    const files = [];
    const automations = "/config/automations.yaml";
    if (await fileExists(automations))
        files.push(automations);
    ...
    if (await fileExists(packagesDir)) {
        ...
        for (const name of lvl1) {
            ...
            if (!st)
                continue;
            if (st.isFile() && (name.endsWith(".yaml") || name.endsWith(".yml")))
                files.push(p);
            if (st.isDirectory()) {
                const lvl2 = await fs.readdir(p).catch(() => []);
                for (const n2 of lvl2) {
                    if (n2.endsWith(".yaml") || n2.endsWith(".yml"))
                        files.push(path.join(p, n2));
                }
            }
        }
    }
...
function extractAutomationLists(doc) {
...
async function findAutomationInYaml(itemId) {
...
/************************/
const transports = {};
function requireTransport(req, res) {
