...
export function registerTools(mcp) {
  defineTool(mcp, {
    name: "ha_get_state",
    description:
      "Get the current state and attributes of a Home Assistant entity by entity_id.",
    params: {
      entity_id: z
        .string()
        .min(1)
        .describe("The entity_id to query (e.g., 'light.living_room')"),
    },
  ...
  defineTool(mcp, {
    ...
    description:
      "Get host/system information from the Home Assistant Supervisor (works only when using Supervisor proxy mode).",
    handler: async () => ({ host_info: await supervisorHostInfo() }),
  });
  defineTool(mcp, {
    name: "diagnose_entity",
    description:
      "Return a compact diagnostic summary for an entity, including availability and last update times.",
    params: {
      entity_id: z.string().min(1).describe("The entity_id to diagnose"),
    },
  ...
  defineTool(mcp, {
    ...
    params: {
      automation_entity_id: z
        .string()
        .min(1)
        .describe("The automation entity_id to diagnose"),
      since_hours: z
        .number()
        .min(1)
        .max(168)
        .optional()
        .describe("Number of hours to look back (default: 24, max: 168)"),
      include_logbook: z
        .boolean()
        .optional()
        .describe("Include logbook entries (default: true)"),
      include_history: z
        .boolean()
        .optional()
        .describe("Include history data (default: false)"),
      include_raw_traces: z
        .boolean()
        .optional()
        .describe("Include raw trace data (default: false)"),
      include_config: z
        .boolean()
        .optional()
        .describe("Include automation configuration (default: true)"),
      include_raw_config: z
        .boolean()
        .optional()
        .describe("Include unredacted configuration (default: false)"),
    },
    handler: async ({
      ...
      const startIso = toIsoFromMillis(
      ...
      if (include_logbook ?? true) {
        try {
          logbook = await haLogbook({
            startIso,
            endIso,
            entityId: automation_entity_id,
          });
        } catch (e) {
      ...
      if (include_history ?? false) {
        try {
          history = await haHistoryPeriod({
            startIso,
            endIso,
            entityIds: [automation_entity_id],
          });
        } catch (e) {
      ...
      return {
        ...
        evidence: {
          trace_sample: trace
            ? {
                timestamp:
                  trace?.timestamp ?? trace?.time ?? trace?.created ?? null,
                result: trace?.result ?? null,
                error: trace?.error ?? trace?.result?.error ?? null,
                failed_step:
                  trace?.failed_step ?? trace?.result?.failed_step ?? null,
              }
            : null,
          raw_traces: include_raw_traces ? traces : undefined,
          logbook_sample: Array.isArray(logbook)
            ? logbook.slice(0, 20)
            : logbook,
  ...
  defineTool(mcp, {
    ...
    params: {
      automation_entity_id: z
        .string()
        .min(1)
        .describe("The automation entity_id"),
    },
  ...
  defineTool(mcp, {
    ...
    params: {
      ...
      include_raw_config: z
        .boolean()
        .optional()
        .describe("Return unredacted config (default: false)"),
    },
  ...
  defineTool(mcp, {
    ...
    params: {
      query: z
        .string()
        .min(1)
        .describe("Search query to match against entity_id and friendly_name"),
      domains: z
        .array(z.string().min(1))
        .optional()
        .describe(
          "Optional array of domains to filter by (e.g., ['automation', 'light'])"
        ),
      limit: z
        .number()
        .min(1)
  ...
  defineTool(mcp, {
    ...
    description:
      "List Home Assistant entities, optionally filtered by domain (e.g. automation, light, sensor).",
    params: {
      domain: z
        .string()
        .min(1)
        .optional()
        .describe(
          "Optional domain filter (e.g., 'automation', 'light', 'sensor')"
        ),
      limit: z
        .number()
        .min(1)
    ...
    handler: async ({ domain, limit }) => {
      ...
      const results = states
        .filter((s) =>
          !domain ? true : String(s?.entity_id ?? "").startsWith(domain + ".")
        )
        .slice(0, lim)
        .map((s) => ({
          entity_id: s.entity_id,
      ...
      return {
        ...
        note:
          states.length > lim
            ? `Truncated to ${lim}. Increase limit if needed.`
            : null,
      };
    },
  });
  defineTool(mcp, {
    ...
    description:
      "Fetch the YAML definition of an automation by item id (fallback for YAML-managed automations).",
    params: {
      automation_entity_id: z
        .string()
        .min(1)
        .describe("The automation entity_id or item id"),
    },
    handler: async ({ automation_entity_id }) => {
      ...
      const itemId = automation_entity_id.startsWith("automation.")
        ...
        : automation_entity_id;
      const r = await fetch(
        `${addonUrl}/yaml/automation/${encodeURIComponent(itemId)}`
      );
      if (!r.ok) {
